...
if (journey) {
  ...
  function draw() {
    ...
    const raw = Math.max(0, Math.min(6, -rect.top / unit));
    ...
    const camp = reduced.matches ? 0 : smooth((raw - 2.15) / .65) * (1 - smooth((raw - 3.55) / .45));
    const presentation = reduced.matches ? 0 : smooth((raw - 4.35) / .6);
    ...
    crew.forEach((dot, i) => {
      ...
      dot.style.opacity = String(1 - (reduced.matches ? 0 : smooth((raw - 5.5) / .5) * .65));
    });
    stars.style.opacity = String(camp * .8);
    night.style.opacity = String(camp * .6);
    ideas.style.opacity = String(camp);
    ideas.setAttribute('transform', `translate(0 ${10 * (1 - camp)})`);
    demo.style.opacity = String(presentation);
    const sunY = 300 + camp * 180 - (reduced.matches ? 0 : smooth((raw - 5) / 1) * 120);
    sun.setAttribute('cy', sunY);
    glow.setAttribute('cy', sunY);
    sun.style.opacity = String(.65 * (1 - camp));
    mist.setAttribute('transform', `translate(${reduced.matches ? 0 : raw * 8} 0)`);
    caption.textContent = reduced.matches ? 'Valley → Mountain → Valley' : ['The journey / One weekly cycle', 'The valley / Meet the crew', 'The ascent / Leave devices behind', 'Base camp / Pen & paper', 'The valley / 24 hours to build', 'Demo Day / Show it', 'The way home / Keep the ideas'][Math.min(6, Math.round(raw))];
  }
