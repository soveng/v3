...
export function generateProjects({ write, layout, escape: e }) {
  ...
  const summary = (c, compact = false) => `<a class="cohort-tile${compact ? " compact" : ""}" href="/projects/${c}/"><span class="cohort-code">${c}</span><h3>${e(intros[c]?.theme || c)}</h3><p>Explore the work <span aria-hidden="true">↗</span></p></a>`;
