...
export function generateProjects({ write, layout, escape: e }) {
  ...
  const directory = (list, overview) => `<section class="archive-explorer" aria-labelledby="directory-title">
    <div class="archive-section-heading"><h2 id="directory-title">${overview ? "Explore the work." : "Built in this cohort."}</h2><p>${list.length} archive entries</p></div>
  ...
  write("projects/index.html", layout("Project archive", "Explore projects built across Sovereign Engineering cohorts.", "/projects/", `<div class="project-archive">
    <header class="projects-hero"><div><p class="section-index">Sovereign Engineering / Project archive</p><h1>Built here.<br><em>Out there.</em></h1><p class="content-lead">Tools, protocols, and experiments from builders who came together to ship something real.</p></div><div class="archive-totals"><strong>${projects.length}</strong><span>projects across ${cohorts.length} cohorts</span><a class="text-link" href="#directory-title">Explore the archive ↓</a></div></header>
  ...
  for (const [index, cohort] of cohorts.entries()) {
    ...
    write(`projects/${cohort}/index.html`, layout(`${cohort} — ${intros[cohort]?.theme || "Projects"}`, `Projects built during ${cohort} at Sovereign Engineering.`, `/projects/${cohort}/`, `<div class="project-archive"><a class="text-link" href="/projects/">← Project archive</a><header class="cohort-hero"><p class="section-index">${cohort} / ${entries(cohort).length} projects</p><h1>${e(intros[cohort]?.theme || cohort)}<span class="cohort-ghost" aria-hidden="true">${cohort.slice(-2)}</span></h1><p class="content-lead">${intro(cohort)}</p></header>${pager}${directory(entries(cohort), false)}${pager}${scripts}</div>`, "/images/sovereign-engineering.png"));
  }
