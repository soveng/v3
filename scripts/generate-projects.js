...
export function generateProjects({ write, layout, escape: e }) {
  ...
  for (const [index, story] of stories.entries()) {
    ...
    write(`projects/${story.slug}/index.html`, layout(story.name, story.intro, `/projects/${story.slug}/`, `<div class="project-story"><a class="text-link" href="/projects/">← Project archive</a>
      <header class="project-story-hero"><div><p class="section-index">Sovereign Engineering / ${e(story.category || first || "Project")}</p><h1>${e(story.name)}</h1><p class="project-tagline">${e(story.tagline)}</p><p class="content-lead">${e(story.intro)}</p></div>${heroArt(story.name, story.logo)}</header>
      ...
      <a class="next-project" href="/projects/${next.slug}/"><span class="section-index">Keep exploring</span><strong>${e(next.name)} ↗</strong></a>${scripts}</div>`, story.logo || "/images/sovereign-engineering.png"));
  }
}
