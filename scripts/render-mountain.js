...
export function renderMountain() {
  ...
  return `<div class="mountain-page">
    <div class="mountain-journey">
      ...
      <div class="mountain-chapters">${data.chapters.map((chapter,i)=>`<section class="mountain-chapter" id="mountain-step-${i}" data-step="${i}" aria-labelledby="mountain-heading-${i}"><div class="mountain-copy"><p class="mountain-day">${e(chapter.day)}</p>${i===0?`<h1 id="mountain-heading-${i}">${e(chapter.title).replace(/(\S+)$/, "<em>$1</em>").replace(". ", ".<br>")}</h1>`:`<h2 id="mountain-heading-${i}">${e(chapter.title)}</h2>`}<p class="mountain-description">${e(chapter.body)}</p>${chapter.detail ? `<p class="mountain-chapter-detail">${e(chapter.detail)}</p>` : ""}${i===0?'<a class="mountain-scroll" href="#mountain-step-1">Follow the trail <span aria-hidden="true">↓</span></a>':''}</div></section>`).join('')}</div>
    </div>
    <section class="mountain-outro"><p class="mountain-day">Mountain cohort / Spring 2027</p><h2>Keep the<br>dialogue going.</h2><p>Dialogical development starts with thinking together. A week of shared questions, unexpected connections, and ideas we can build on long after we leave the mountain.</p><p class="mountain-details">Dates, location, and application details will follow.</p><a class="text-link" href="/faq/">About Sovereign Engineering ↗</a></section>
